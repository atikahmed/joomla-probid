...
class RSTicketsProModelPriorities extends JModel
{
	var $_data = null;
	var $_total = 0;
	var $_query = '';
	...
	function _buildQuery()
	{
		...
		$query = "SELECT * FROM #__rsticketspro_priorities WHERE 1";
	...
	function getPriorities()
	{
		...
		if (empty($this->_data))
			$this->_data = $this->_getList($this->_query, $this->getState($option.'.priorities.limitstart'), $this->getState($option.'.priorities.limit'));
	...
	function getPagination()
	{
		if (empty($this->_pagination))
		{
			...
			$this->_pagination = new JPagination($this->getTotal(), $this->getState($option.'.priorities.limitstart'), $this->getState($option.'.priorities.limit'));
		}
	...
	function publish($cid=array(), $publish=1)
	{
		if (!is_array($cid) || count($cid) > 0)
		{
			...
			$query = "UPDATE #__rsticketspro_priorities SET `published`='".$publish."' WHERE `id` IN (".$cids.")";
	...
	function remove($cids)
	{
		...
		$this->_db->setQuery("DELETE FROM #__rsticketspro_priorities WHERE `id` IN (".$cids.")");
		$this->_db->query();
		
		$this->_db->setQuery("UPDATE #__rsticketspro_tickets SET priority_id=3 WHERE priority_id IN (".$cids.")");
		$this->_db->query();
