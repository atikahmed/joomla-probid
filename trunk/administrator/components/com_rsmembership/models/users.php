...
class RSMembershipModelUsers extends JModel
{
	...
	function getUser()
	{
		...
		{
			$this->_db->setQuery("SELECT * FROM #__rsmembership_transactions WHERE `id`='".$temp."'");
			$transaction = $this->_db->loadObject();
			
			$data = @unserialize($transaction->user_data);
			...
			if ($data)
			{
				...
				if ($data->username)
					...
					$user->set('username', JText::_('RSM_USERNAME_EMPTY'));
				$user->set('name', $data->name);
				$user->set('email', $transaction->user_email);
			}
		}
		...
		$row->transactions = $this->_getList("SELECT t.* FROM #__rsmembership_transactions t WHERE `user_id`='".$cid."' ORDER BY `date` DESC");
	...
	{
		if (empty($this->_memberships))
			$this->_memberships = $this->_getList("SELECT c.name AS category_name, m.* FROM #__rsmembership_memberships m LEFT JOIN #__rsmembership_categories c ON (c.id=m.category_id) WHERE m.`published`='1' ORDER BY c. ordering, m.ordering");
	...
	function getTempId()
	{
		return JRequest::getInt('temp_id');
	}
}
?>