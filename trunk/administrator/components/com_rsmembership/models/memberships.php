...
class RSMembershipModelMemberships extends JModel
{
	...
	var $attachmentsPagination = array();
	...
	function _buildQuery()
	{
		...
		$query = "SELECT m.*, c.name AS category_name FROM #__rsmembership_memberships m LEFT JOIN #__rsmembership_categories c ON (m.category_id=c.id) WHERE 1";

		$filter_word = JRequest::getString('search', '');
		if (!empty($filter_word))
			$query .= " AND m.`name` LIKE '%".$filter_word."%'";
		...
		if ($filter_state != '')
			$query .= " AND m.`published`='".($filter_state == 'U' ? '0' : '1')."'";
		
		$sortColumn = JRequest::getVar('filter_order', 'm.ordering');
	...
	function copy($cid)
	{
		...
		$this->_db->setQuery("SELECT * FROM #__rsmembership_membership_attachments WHERE `membership_id`='".$cid."'");
		$attachments = $this->_db->loadObjectList();
		...
		$this->_db->setQuery("SELECT * FROM #__rsmembership_membership_extras WHERE `membership_id`='".$cid."'");
		...
		$this->_db->setQuery("SELECT * FROM #__rsmembership_membership_shared WHERE `membership_id`='".$cid."'");
	...
	function getDays()
	{
		...
		$tmp = new stdClass();
		$tmp->value = 0;
		$tmp->text  = JText::_('RSM_EVERY_DAY');
	...
	function getMonths()
	{
		...
		$tmp = new stdClass();
		$tmp->value = 0;
		$tmp->text  = JText::_('RSM_EVERY_MONTH');
