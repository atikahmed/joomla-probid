...
<form action="<?php echo JRoute::_($this->link); ?>" method="post" name="adminForm" id="adminForm" enctype="multipart/form-data">
<?php if (!empty($this->function) && $this->function == 'addmembershipshared') { ?>
...
<?php } ?>
<?php if ($this->params->show_add) { ?>
<button id="folder_add_button" type="button" onclick="if(document.adminForm.boxchecked.value==0){alert('<?php echo JText::sprintf('RSM_PLEASE_MAKE_A_SELECTION_FROM_THE_LIST_TO', JText::_('RSM_ADD')); ?>');}else{submitbutton()}"><?php echo JText::_('RSM_ADD_SELECTED_ITEMS'); ?></button>
<?php } ?>
