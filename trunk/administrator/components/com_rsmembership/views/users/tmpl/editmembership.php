...
{
	<?php if (!$this->row->id) { ?>
		var membership_id = $('membership_id').value;
		<?php foreach ($this->periods as $period) { ?>
		if (membership_id == <?php echo $period->membership_id; ?>)
		{
			<?php if ($period->offset) { ?>
			$('change_unlimited').checked = false;
			rsmembership_change_unlimited($('change_unlimited'));
			rsmembership_date(<?php echo $this->now + $period->offset; ?>, 'membership_end');
			$('membership_end').value = '<?php echo $this->now + $period->offset; ?>';
			<?php } else { ?>
			$('change_unlimited').checked = true;
			rsmembership_change_unlimited($('change_unlimited'));
			<?php } ?>
		}
		<?php } ?>
...
<form action="<?php echo JRoute::_('index.php?option=com_rsmembership&controller=users&task=editmembership&tmpl=component&cid='.$this->row->user_id); ?>" method="post" name="adminForm" id="adminForm">
	<table cellspacing="0" cellpadding="0" border="0" width="100%" class="adminform">
		...
		<tr>
			...
			<td><span id="membership_start_ajax"><?php echo date(RSMembershipHelper::getConfig('date_format'), RSMembershipHelper::getCurrentDate($this->row->membership_start)); ?></span> <?php echo JHTML::calendar($this->row->membership_start, 'membership_start', 'membership_start', '%s', 'onchange="rsmembership_date(this.value,\'membership_start\')" style="display: none"'); ?></td>
		</tr>
		<tr>
			...
			<td><span id="membership_end_ajax"><?php echo date(RSMembershipHelper::getConfig('date_format'), RSMembershipHelper::getCurrentDate($this->row->membership_end)); ?></span> <?php echo JHTML::calendar($this->row->membership_end, 'membership_end', 'membership_end', '%s', 'onchange="rsmembership_date(this.value,\'membership_end\')" style="display: none"'); ?>
			<input type="checkbox" name="unlimited" id="change_unlimited" value="1" onchange="rsmembership_change_unlimited(this);" <?php echo $this->row->membership_end == 0 ? 'checked="checked"' : ''; ?> /> <label for="change_unlimited"><?php echo JText::_('RSM_UNLIMITED'); ?></label>
			</td>
		</tr>
		<tr>
