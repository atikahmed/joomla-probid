...
<table class="adminlist" id="addmemberships">
	...
	?>
		<tr class="row<?php echo $k; ?>">
			...
			<td><a href="<?php echo JRoute::_('index.php?option=com_rsmembership&controller=users&task=editmembership&tmpl=component&cid='.$row->id); ?>" class="modal" rel="{handler: 'iframe', size: {x: 660, y: 475}}"><?php echo $row->name != '' ? $row->name : JText::_('RSM_NO_TITLE'); ?></a></td>
			<td>
				<?php echo RSMembershipHelper::getPriceFormat($row->price); ?>
			</td>
			<td><?php echo date(RSMembershipHelper::getConfig('date_format'), RSMembershipHelper::getCurrentDate($row->membership_start)); ?></td>
			<td><?php echo $row->membership_end > 0 ? date(RSMembershipHelper::getConfig('date_format'), RSMembershipHelper::getCurrentDate($row->membership_end)) : JText::_('RSM_UNLIMITED'); ?></td>
