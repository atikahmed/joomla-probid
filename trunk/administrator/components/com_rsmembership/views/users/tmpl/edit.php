...
function reloadSqueezeBox()
{
	...
	<?php } else { ?>
	$$('a.modal').each(function(el) { el.addEvent('click', function(e) { new Event(e).stop(); SqueezeBox.fromElement(el); }); });
	<?php } ?>
}
</script>

<form action="<?php echo JRoute::_($this->temp ? 'index.php?option=com_rsmembership&view=transactions' : 'index.php?option=com_rsmembership&controller=users&task=edit'); ?>" method="post" name="adminForm" id="adminForm">
...
<?php echo $this->tabs->startPanel(JText::_('RSM_USER_INFO'), 'user-info'); ?>
	<table cellspacing="0" cellpadding="0" border="0" width="100%" class="adminform">
		<tr>
			...
			<?php } else { ?>
			<td><?php echo $this->escape($this->row->username); ?></td>
			<?php } ?>
		</tr>
		<tr>
			...
			<?php } else { ?>
			<td><?php echo $this->escape($this->row->name); ?></td>
			<?php } ?>
		</tr>
		<tr>
			...
			<?php } else { ?>
			<td><?php echo $this->escape($this->row->email); ?></td>
			<?php } ?>
		</tr>
...
<thead>
	...
	<tr class="row<?php echo $k; ?>">
		...
		<td><?php echo date(RSMembershipHelper::getConfig('date_format'), RSMembershipHelper::getCurrentDate($row->date)); ?></td>
