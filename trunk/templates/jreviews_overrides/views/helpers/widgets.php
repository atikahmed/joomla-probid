...
class WidgetsHelper extends MyHelper
{
	...
	//Function modified by FDS-GT
    function favorite($listing)
    {
        $output = '';
        $listing_id = $listing['Listing']['listing_id'];
		$directoryTitle = $listing['Directory']['title'];
		$strProject  = 'Project';
		$pos = strpos($directoryTitle, $strProject);
        //$User = cmsFramework::getUser();
		...
		$db->setQuery( $query );
		$queryInArray = $db->loadObjectList();
		$listing_type = $queryInArray[0]->listing_type;
		
		//IF this is a project (type 2)
		if($listing_type==2){
			$buttonTitle = 'Projects of Interest';
		} else {
			$buttonTitle = 'Preferred/Favorites';
		}
		
		//This file grabs the User Group ID
		include("checkGroupID.php");
		
        $output .= '<span class="jrFavoriteWidget" title="' . __t("Preferred/Favorites", true) . '">';
        $output .= '<span class="jrIcon jrIconFavorites"></span>';
        $output .= '<span id="jr_favoriteCount' . $listing_id . '">' . $listing['Favorite']['favored']  . '</span>';
        $output .= '</span>';
        
		//IF this is a PO viewing a Project, no button. 
		if(($groupID==13 || $groupID==19) && $pos!==false) {
		}
        elseif($listing['Favorite']['my_favorite']) { // Already in user's favorites
            $output .= '<span id="jr_favoriteImg' . $listing_id . '" class="jrFavoriteButton jrButton" title="' . __t("Remove from " .$buttonTitle, true) . '" onclick="jreviews.favorite.remove(this,{listing_id:' . $listing_id . '})">' . __t("Remove", true) . '</span>';
        }
        elseif($User->id) { // Not in user's favorites
            $output .= '<span id="jr_favoriteImg' . $listing_id . '" class="jrFavoriteButton jrButton" title="' . __t("Add to " .$buttonTitle, true) . '" onclick="jreviews.favorite.add(this,{listing_id:' . $listing_id . '})">' . __t("Add", true) . '</span>';
        }
        else { // This is a guest user, needs to register to use the favorites widget
            $output .= '<span id="jr_favoriteImg' . $listing_id . '" class="jrFavoriteButton jrButton" title="' . __t("Add to Preferred/Favorites", true) . '" onclick="s2Alert(\''.__t("Login or registration is required to add this entry to your Preferred/Favorites",true).'\');">' . __t("Add", true) . '</span>';    
        }
