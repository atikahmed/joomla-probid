...
var MooTooltips = new Class({
	...
	initialize: function(options) {
		...
		e.each(function(el){
			$(el.id).setProperty( 'rel', JSON.encode(el) );
	...
	attach: function(){
		this.elements.each(function(elem, key){
			...
			*/
			...
			{
				elem.addEvent(endEvent, function(event) {
					var tip = elem.retrieve('tip');
					var elProperties = tip.retrieve('properties');
					var leave = elProperties.get('leave');
					tooltip.container.morph({'opacity':0,'top': leave});
				});
			}
			else
	...
	enter: function(event, element){
		...
		if( elProperties.ajax && !elProperties.loaded ){
			new Request.HTML({
				...
				update: tip.getElement('.rsticketspro_tip_message'),
	...
	createContainer: function( sticky ){
		...
		var message = new Element('div', {'class':'rsticketspro_tip_message'});
