...
$this->_db->setQuery($query);
$indexes = $this->_db->loadResultArray();
...
if($this->cmsVersion == CMS_JOOMLA15 && !in_array('jr_listing_count',$indexes)) {
	$query = "ALTER TABLE `#__content` ADD INDEX `jr_listing_count` ( `sectionid` , `catid` , `state` , `access` , `publish_up` , `publish_down` );";
	$this->_db->setQuery($query);
	$this->_db->query();
	usleep(1000000);
} 
elseif(!in_array('jr_listing_count',$indexes)) {
