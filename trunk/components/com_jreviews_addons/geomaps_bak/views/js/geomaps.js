...
var geomapsLanguage = new Array(); 
geomapsLanguage["no_streetview"] = jrLanguage["geomaps.no_streeview"];
geomapsLanguage["cannot_geocode"] = jrLanguage["geomaps.cannot_geocode"];
geomapsLanguage["drag_marker"] = jrLanguage["geomaps.drag_marker"];
geomapsLanguage["directions_bad_address"] = jrLanguage["geomaps.directions_bad_address"];
geomapsLanguage["directions_request_error"] = jrLanguage["geomaps.directions_request_error"];
...
geomaps = 
{
    ...
    getAddress: function()
    {
        ...
        jQuery.each(geoAddressObj, function(index, item){ 
            var $input = jQuery('#'+item,'#jr_listingForm');
            ...
            switch(type) {
                case 'select-one': 
                    inputVal = jQuery('#'+item + ' option:selected','#jr_listingForm').text()
                break;
                default:
                    inputVal = jQuery('#'+item,'#jr_listingForm').val();
                break;
            }
...
//        if(null==geomaps.map) geomaps.initializeMap(0,0);
