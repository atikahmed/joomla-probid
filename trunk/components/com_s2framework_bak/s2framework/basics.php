...
function getAjaxUri($app='jreviews') 
{
    ...
    {
            $ajaxUri = WWW_ROOT . 'index.php?option='.S2Paths::get($app, 'S2_CMSCOMP').'&format=raw&tmpl=component';        
    }

    // Required for Joomfish compat in ajax requests
    if($lang = Sanitize::getString($_REQUEST,'lang')) {
        $ajaxUri .= '&lang='.$lang;
    }
