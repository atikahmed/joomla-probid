...
class S2Object {
    ...
    {                                   
        ...
        # Run an additional set of checks to dynamically load j15 themes if present
        if($this->cmsVersion == CMS_JOOMLA15 && !$this->file_prefix)
        {
            $this->file_prefix = true; 
            $path = $this->locateThemeFile($action,'j15.'.$file,$ext,$forceFrontEnd); 
            if($path) return $path;   
        }
...
//        echo $location.DS.$this->viewTheme.DS.$action.DS.$file.$this->viewSuffix.$ext.'<br />';
    ...
    {
        if(substr($file,-3) != '.js') $file = $file.'.js';
