...
class RSMembershipModelUser extends JModel
{
	...
	function _bindData($verbose=true)
	{
		...
		foreach ($fields as $field)
			if (($field->required && empty($post['rsm_fields'][$field->name])) || ($field->rule && is_callable('RSMembershipValidation', $field->rule) && !call_user_func(array('RSMembershipValidation', $field->rule), @$post['rsm_fields'][$field->name])))
			{
