...
class ModRSFormListHelper
{
	...
	function _buildQuery()
	{
        ...
        /**
         * JOEYG ADDED CODE TO GET WALL POSTS ONLY FOR THIS ARTICLE ID
         */
        $article_id = JRequest::getInt('id');
        $query .= " AND sv.FieldName = 'article_id'";
		$query .= " AND sv.FieldValue = '". $article_id ."'";


        require_once(JPATH_THEMES.DS.'jreviews_overrides' . DS . 'helpers' . DS . 'wall.php');
        $wall = new JGWallHelper();
        //IF NOT OWNER THAN DO NOT SHOW SYSTEM NOTES
        if (!$wall->isOwner()) {
           $query .= " AND sv.SubmissionId NOT IN ";
           $query .= "(SELECT `SubmissionId` FROM  `jos_rsform_submission_values` WHERE  `FieldValue` =  'note')";
        }
        //ENDS JOEYG CUSTOM SQL CODE
