...
<form action="<?php echo JRoute::_('index.php?option=com_rsticketspro&view=kbrules'); ?>" method="post" name="adminForm" id="adminForm">
	...
	<div id="editcell1">
		<table class="adminlist">
			...
			<tr>
				...
				<th width="20"><input type="checkbox" name="toggle" value="" onclick="checkAll(<?php echo count($this->kbrules); ?>);"/></th>
				<th><?php echo JHTML::_('grid.sort', 'RST_KB_RULE_NAME', 'r.name', $this->sortOrder, $this->sortColumn); ?></th>
	...
	$k = 0;
	$i = 0;
	$n = count($this->kbrules);
	foreach ($this->kbrules as $row)
	{
	...
	?>
		<tr class="row<?php echo $k; ?>">
			...
			<td><a href="<?php echo JRoute::_('index.php?option=com_rsticketspro&controller=kbrules&task=edit&cid='.$row->id); ?>"><?php echo $row->name != '' ? $this->escape($row->name) : JText::_('RST_NO_TITLE'); ?></a></td>
			...
			<?php } else { ?>
				<?php echo $row->category != '' ? $row->category : JText::_('RST_NO_TITLE'); ?>
			<?php } ?>
			</td>
