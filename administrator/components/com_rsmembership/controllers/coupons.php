...
class RSMembershipControllerCoupons extends RSMembershipController
{
	...
	function saveOrder()
	{
		...
		// Redirect
		$this->setRedirect('index.php?option=com_rsmembership&view=coupons', JText::_('RSM_COUPONS_ORDERED'));
	}
	...
	function move() 
	{
		...
		// Redirect
		$this->setRedirect('index.php?option=com_rsmembership&view=coupons');
	}
	...
	function changestatus()
	{
		...
		else
		{
			...
			$total = count($cid);
			if ($value)
				$msg = JText::sprintf('RSM_COUPONS_PUBLISHED', $total);
			else
				$msg = JText::sprintf('RSM_COUPONS_UNPUBLISHED', $total);
		...
		// Redirect
		$this->setRedirect('index.php?option=com_rsmembership&view=coupons', $msg);
	}
	...
	function remove()
	{
		...
		else
		{
			...
			$total = count($cid);
			$msg = JText::sprintf('RSM_COUPONS_DELETED', $total);
	...
	function save()
	{
		...
		{
			case 'apply':
				$link = 'index.php?option=com_rsmembership&controller=coupons&task=edit&cid='.$cid;
				if ($result)
					$this->setRedirect($link, JText::_('RSM_COUPON_SAVED_OK'));
				else
					$this->setRedirect($link, JText::_('RSM_COUPON_SAVED_ERROR'));
			break;
		
			case 'save':
				$link = 'index.php?option=com_rsmembership&view=coupons';
				if ($result)
					$this->setRedirect($link, JText::_('RSM_COUPON_SAVED_OK'));
				else
					$this->setRedirect($link, JText::_('RSM_COUPON_SAVED_ERROR'));
