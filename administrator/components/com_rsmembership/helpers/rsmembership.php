...
class RSMembershipHelper
{
	...
	{
		static $rsmembership_config;
		
		if (!is_object($rsmembership_config) || $reload)
		{
		...
		return $rsmembership_config;
	}
	
	function getConfig($name = null)
	{
		$config = RSMembershipHelper::readConfig();
	...
	function getCurrentDate($date=null)
	{
		...
		{			
			...
			$unix = is_null($date) ? time() : $date;
		}
		else
		{
			$date = JFactory::getDate($date, -$config->getValue('offset'));
			$unix = $date->toUnix();
		}
	...
	function checkPatches($type)
	{
		...
		$file = RSMembershipHelper::getPatchFile($type);
	...
	function getModulesWhere()
	{
		...
		if ($not_allowed)
			return (!RSMembershipHelper::isJ16() ? " AND" : "")." m.id NOT IN (".implode(',', $not_allowed).")";
	...
	function sendExpirationEmails()
	{
		...
		if ($now < RSMembershipHelper::getConfig('expire_last_run') + RSMembershipHelper::getConfig('expire_check_in')*60)
			return;
		
		$db->setQuery("UPDATE #__rsmembership_configuration SET `value`='".$now."' WHERE `name`='expire_last_run'");
		$db->query();
		...
		$db->setQuery("SELECT * FROM #__rsmembership_memberships WHERE (`user_email_from_addr`!='' OR `user_email_use_global`='1' OR `admin_email_to_addr` != '') AND published='1'");
		...
		foreach ($memberships as $membership)
		{
			...
			foreach ($results as $result)
			{
				...
				if ($membership->user_email_expire_subject)
				{
					...
					// from address
					$from = $membership->user_email_use_global ? $jconfig->getValue('config.mailfrom') : $membership->user_email_from_addr;
					// from name
					$fromName = $membership->user_email_use_global ? $jconfig->getValue('config.fromname') : $membership->user_email_from;
					// recipient
					...
					$bcc = null;
					// attachments
					$db->setQuery("SELECT `path` FROM #__rsmembership_membership_attachments WHERE `membership_id`='".$membership->id."' AND `email_type`='user_email_expire' AND `published`='1' ORDER BY `ordering`");
	...
	function getUserSubscriptions()
	{
		...
		if (!$user->get('guest'))
		{
			$db->setQuery("SELECT `membership_id`, `extras` FROM #__rsmembership_membership_users WHERE `user_id`='".$user->get('id')."' AND `status`='0'");
			$results = $db->loadObjectList();
	...
	function getShared($type=null, $search=null)
	{
		...
		$db->setQuery("SELECT s.`membership_id`, s.`params`, s.`type` FROM #__rsmembership_membership_shared s LEFT JOIN #__rsmembership_memberships m ON (s.membership_id=m.id) WHERE $where AND s.`published`='1' AND m.published='1'");
		$shared = $db->loadObjectList();
		
		$db->setQuery("SELECT s.`extra_value_id`, s.`params`, s.`type` FROM #__rsmembership_extra_value_shared s LEFT JOIN #__rsmembership_extra_values v ON (s.extra_value_id=v.id) WHERE $where AND s.`published`='1' AND v.published='1'");
		$shared2 = $db->loadObjectList();
		if (!empty($shared2))
			$shared = array_merge($shared, $shared2);
...
class RSMembership
{
	...
	function saveTransactionLog($log, $id, $append=true)
	{
		...
		if ($append)
			$db->setQuery("UPDATE #__rsmembership_transactions SET `response_log` = CONCAT(`response_log`,'".$db->getEscaped("\n".$log)."') WHERE `id`='".(int) $id."'");
		else
			$db->setQuery("UPDATE #__rsmembership_transactions SET `response_log`='".$db->getEscaped($log)."' WHERE `id`='".(int) $id."'");
	...
	{
		$db =& JFactory::getDBO();
		$db->setQuery("UPDATE #__rsmembership_transactions SET `status`='denied' WHERE `id`='".(int) $transaction_id."'".($force ? "" : " AND `status`='pending'"));
		return $db->query();
	}
	...
	function sendUserEmail(&$user, $password, $fields, $new_user=true)
	{
		$mainframe =& JFactory::getApplication();
		...
		//BEGIN added by FDS - GT
		// Optionally add embedded image
		$image = '<img src="' .$siteURL. 'images/email_1A.jpg" />';
		//END added by FDS - GT

		if ($useractivation == 1 && $new_user)
		{
			if (RSMembershipHelper::isJ16())
			...
			$message = JText::sprintf('RSM_NEW_EMAIL_ACTIVATE', $name, $sitename, $activation_url, $siteURL, $username, $password, $image);
			//BEGIN added by FDS - GT
			//SEE language file /language/en-GB/en-GB.com_rsmembership.ini
			$message .= JText::sprintf('PBD_EMAIL_SIGNATURE', $sitename);
			$message .= JText::sprintf('PBD_EMAIL_DISCLOSURE');
			//END added by FDS - GT
		}
		else
	...
	function createUserData($user_id, $post)
	{
		...
		$db->setQuery("SELECT `user_id` FROM #__rsmembership_users WHERE `user_id`='".$user_id."'");
		if (!$db->loadResult())
		{
			$db->setQuery("INSERT INTO #__rsmembership_users SET `user_id`='".$user_id."'");
			$db->query();
		}
		...
		$db->setQuery("UPDATE #__rsmembership_users SET ".implode(", ", $columns)." WHERE user_id='".$user_id."' LIMIT 1");		
		$db->query();
	}
	...
	{
		$db =& JFactory::getDBO();
		$db->setQuery("UPDATE #__users SET `block`='0' WHERE `id`='".$user_id."' AND `block`='1' AND `lastvisitDate` != '0000-00-00 00:00:00'");
		$db->query();
