...
class RSMembershipModelFiles extends JModel
{
	...
	function addsubscriberfiles($files)
	{
		...
		{
			$email_type = JRequest::getCmd('email_type');
			...
			$this->_db->setQuery("SELECT id FROM #__rsmembership_membership_attachments WHERE `path`='".$this->_db->getEscaped($file)."' AND `email_type`='".$this->_db->getEscaped($email_type)."' AND `membership_id`='".$membership_id."'");
			if ($this->_db->loadResult())
				continue;
				
			$row->ordering = $row->getNextOrder("`membership_id`='".$row->membership_id."' AND `email_type`='".$this->_db->getEscaped($email_type)."'");
