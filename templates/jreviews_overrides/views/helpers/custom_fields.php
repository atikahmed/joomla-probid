				...
				if((!$search && $this->Access->in_groups($value['properties']['access'])) || ($search && $this->Access->in_groups($value['properties']['access_view']))) 
                {
                    ...
                    switch($value['type']){
                        ...
                        case 'integer':
							...
							//BEGIN Code added by FDS-GT
							//grab the business ID for Vendors to populate for related listings
							
							//this will show for the input form
							if (!isset($value['selected']) & $value['name'] == 'jr_businessid') {
							
							$m_user =& JFactory::getUser();
							if (!$m_user->guest):
							$db = JFactory::getDbo();
							$query = "SELECT `id` FROM `jos_vpbd_content` WHERE `catid` = 80 AND `created_by` = " .$m_user->id;
							$db->setQuery($query);
							...
							//echo 'you are here 1';
							$inputs["data[Field][$fieldLocation][$key]"]['class'] = 'jr_hidden';
							$value['selected'] = $myBusinessID;}
							
							//this will show for the edit form
							elseif ($value['name'] == 'jr_businessid') {
							//echo 'you are here 2';
							$inputs["data[Field][$fieldLocation][$key]"]['class'] = 'jr_hidden';}
                        ...
                        case 'website':
							...
							//$value['selected'] = 'http://go.' .$myBusinessID;}
							//END Code added by FDS-GT                     
                        break;
                        case 'email':
