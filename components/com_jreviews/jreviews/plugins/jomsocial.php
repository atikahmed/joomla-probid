...
class JomsocialComponent extends S2Component {
        ...
        if(file_exists($path))
            {
                ...
                $this->Thumbnail = ClassRegistry::getClass('ThumbnailHelper');
    ...
    function _plgDiscussionAfterSave(&$model)
    {
        ...
        if($stream)
        {
            ...
            if(isset($model->isNew) && $post['Discussion']['approved'] == 1)
            {
                ...
                !empty($listing['Listing']['images']) and $activity_thumb = $this->Thumbnail->thumb($listing, 0, array('tn_mode'=>$this->c->Config->jomsocial_tnmode,'location'=>'activity','dimensions'=>array($this->c->Config->jomsocial_tnsize)));                
	...
	function _isPaidListing($listing)
	{
		...
		{			
			App::import('Model','paid_plan_category');
