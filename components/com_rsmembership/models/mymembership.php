...
class RSMembershipModelMymembership extends JModel
{
	...
	function getExtras()
	{
		...
		$has_extras = count($this->getBoughtExtras());
		
		$this->_db->setQuery("SELECT e.* FROM #__rsmembership_membership_extras me LEFT JOIN #__rsmembership_extras e ON (me.extra_id = e.id) WHERE me.membership_id = '".$this->_data->membership_id."' AND e.published = '1'".($has_extras ? " AND e.type NOT IN ('dropdown', 'radio')" : ''));
