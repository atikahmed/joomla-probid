...
{    
    ...
    var $routes = array(
        ...
        'category16'=>'index.php?option=com_jreviews&amp;Itemid=%1$s&amp;url=%2$s/%3$s_c%4$s_m%1$s/',
    ...
    function alphaindex($alpha_title,$alpha_value,$directory,$attributes=array()) 
    {
        if(($this->cmsVersion == CMS_JOOMLA15 && count($directory) > 1) || empty($directory)) {
            ...
            $first = $this->cmsVersion == CMS_JOOMLA15 ? array_shift($directory) : array_shift(array_shift($directory)); 
    ...
    function category() 
    {
       ...
       # Check if there's a menu for this category to prevent duplicate urls
        ...
        if($this->cmsVersion == CMS_JOOMLA15)
        {
            ...
            $url = sprintf($this->routes['category'],$dir_slug,$node['Section']['title'],$cat_slug,$dir_id,$section_id,$cat_id,$menu_id);
