        ...
        foreach($tree as $node)
        {
			...
			if ($current_depth == 1) 
			{
				...
				if (!empty($params['image'])) {
					$image_container = '<div class="contentThumbnail">'. $this->Routes->category($node,array('image'=>true)) . '</div>';
				} 
				else {
					$image_container = '';
				}				
				...
				{
					$result .= '<li class="cat_level' . $current_depth . '" style="width: '. $width .'%; clear: both;">' . $image_container;
				}
				else {
					$result .= '<li class="cat_level' . $current_depth . '" style="width: '. $width .'%;">' . $image_container;
				}
				$first_level_count++;
			}
			else
			{
				$result .= '<li class="cat_level' . $current_depth . '">';
			}
			$result .= '<div class="contentDiv">';
