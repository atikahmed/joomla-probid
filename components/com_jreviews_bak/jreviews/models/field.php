...
//prx($fieldValues);
        ...
        if(!empty($fieldValues)) 
        {
            foreach($fieldValues AS $fieldValue)
            {
                ...
                {
                    $values = array_filter(explode('*',$optionval)); // 
            ...
            foreach($fieldValues AS $fieldValue)
            {
                ...
                foreach($nonInputFieldNames AS $nonInputFieldName) {
                    $fieldValue[$nonInputFieldName] = 'banner';
                }
                    ...
                    if ($key != 'element_id' && $value != '' && isset($rows[$key]))
                    {
                        ...
                        if($rows[$key]->{'Field.type'} != 'date' || ($rows[$key]->{'Field.type'} == 'date' && $value != NULL_DATE)) 
                        {
                            ...
                            if(!in_array($rows[$key]->{'Field.type'},$optionFields) ) 
                            {
                                ...
                                $selOptions = !is_array($value) ? array_filter(explode('*',$value)) : $value;
    ...
    function validate(&$data, $fieldLocation, $Access) 
    {
        ...
        if ($groupids) 
        {
            ...
            foreach ($fields as $field) 
            {
                ...
                if ($Access->in_groups($field['Field']['access']))
                {
